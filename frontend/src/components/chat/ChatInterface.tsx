...
export default function ChatInterface({ currentUserId, enableOrientator = false }: ChatInterfaceProps) {
      ...
      switch (action.type) {
        case 'save':
          const saveResponse = await axios.post(
            `${process.env.NEXT_PUBLIC_API_URL}/api/v1/orientator/save-component`,
            {
  ...
  const loadConversationMessages = useCallback(async () => {
    ...
    try {
      ...
      const endpoint = (enableOrientator && isDefaultMode)
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1/orientator/conversations/${currentConversation.id}/messages`
        : `${process.env.NEXT_PUBLIC_API_URL}/api/v1/conversations/${currentConversation.id}/messages`;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (enableOrientator && isDefaultMode) {
        ...
        // Use Orientator endpoint only for default mode when enabled
        endpoint = `${process.env.NEXT_PUBLIC_API_URL}/api/v1/orientator/message`;
        requestBody = { 
          message: userMessage, 
        ...
        endpoint = `${process.env.NEXT_PUBLIC_API_URL}/api/v1/chat/conversations/send/${conversationId}`;
        requestBody = {
          message: userMessage,
          mode: chatMode
        };
  ...
  const handleArchiveConversation = async () => {
    ...
    try {
      const token = await getToken();
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/chat/conversations/${currentConversation.id}/archive`,
        {},
        {
          headers: {
            'Authorization': `Bearer ${token}`
  ...
  const handleDeleteConversation = async () => {
    ...
    try {
      const token = await getToken();
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/chat/conversations/${currentConversation.id}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
